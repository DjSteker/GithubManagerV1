...
std::string GestorGit::filtrarLogSensitive(const std::string &log) {
	...
	static const std::vector<std::pair<std::string, std::string>> filtros = { { "password=", "[REDACTED]" }, { "Password:", "[REDACTED]" }, { "authentication failed", "[REDACTED AUTH]" }, {
			"could not read Username", "[REDACTED]" }, { "could not read Password", "[REDACTED]" }, };
...
ResultadoOperacionGit GestorGit::clonarRepositorio(const std::string &urlRepositorio, const std::string &directorioDestino, const std::string &token, const std::string &rama) {
	ResultadoOperacionGit resultado { false, "", "" };
	...
	std::string comando;
	if (!rama.empty()) {
		comando = "clone -b \"" + escaparParaComillasDobles(rama) + "\" --single-branch \"" + escaparParaComillasDobles(urlRepositorio) + "\" \"" + escaparParaComillasDobles(directorioDestino) + "\"";
	} else {
		comando = "clone \"" + escaparParaComillasDobles(urlRepositorio) + "\" \"" + escaparParaComillasDobles(directorioDestino) + "\"";
	}
	...
	resultado.salidaCompleta = salida;
	resultado.exito = (codigoSalida == 0);
	resultado.mensaje = resultado.exito ? "Repositorio clonado correctamente" + (rama.empty() ? std::string("") : " (rama '" + rama + "')") : "Error al clonar el repositorio";
	return resultado;
}
...
ResultadoOperacionGit GestorGit::subirCambios(const std::string &directorio, const std::string &rama, const std::string &mensajeCommit, const std::string &token, const std::string &urlOpcional) {
	ResultadoOperacionGit resultado { false, "", "" };
	std::ostringstream log;
	int ec = 0;
	...
	if (!rama.empty() && rama != ramaEfectiva) {
		...
		if (ec == 0) {
			// La rama ya existe localmente: cambiar a ella.
			log << "🔀 Cambiando a la rama '" << rama << "' ...\n";
			std::string salidaCheckout = ejecutarComandoGit("checkout " + rama, repoRoot, "", &ec);
			log << salidaCheckout;
			if (ec == 0) {
				ramaEfectiva = rama;
			} else {
				log << "⚠ No se pudo cambiar a la rama '" << rama << "'; usando '" << ramaEfectiva << "'\n";
			}
		} else {
			// La rama no existe localmente: crearla desde la rama actual.
			log << "🌱 La rama '" << rama << "' no existe; creándola con checkout -b ...\n";
			std::string salidaCheckoutB = ejecutarComandoGit("checkout -b " + rama, repoRoot, "", &ec);
			log << salidaCheckoutB;
			if (ec == 0) {
				ramaEfectiva = rama;
			} else {
				log << "⚠ No se pudo crear la rama '" << rama << "'; usando '" << ramaEfectiva << "'\n";
			}
		}
	}
	...
	// ------------------------------------------------------------------
	std::string statusPorcelain = ejecutarComandoGit("status --porcelain --untracked-files=normal", repoRoot, "", &ec);
	...
	if (!statusPorcelain.empty()) {
		...
		if (ec != 0) {
			...
			bool esNadaQueComitear = lc.find("nothing to commit") != std::string::npos || lc.find("no changes added to commit") != std::string::npos
					|| lc.find("nothing added to commit") != std::string::npos;
	...
	if (ec != 0) {
		...
		if (lc.find("does not match any") != std::string::npos || lc.find("src refspec") != std::string::npos) {
			...
			salidaPush = filtrarLogSensitive(ejecutarComandoGit(cmdPushForce, repoRoot, token, &ec));
			log << salidaPush << "\n";
		}
	}
...
std::vector<std::string> GestorGit::obtenerRamasRemotas(const std::string &urlRepositorio, const std::string &token, std::string *mensajeError) {
