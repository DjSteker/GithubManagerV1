...
void Intefaz::on_select_folder(GtkButton *btn, gpointer data) {
	...
	gtk_file_dialog_select_folder(dialog, win, nullptr, on_select_folder_finish, nullptr);
}
...
void Intefaz::run_git_upload(GtkWidget *boton, gpointer user_data) {
	...
	if (rama_final.empty()) {
		...
		if (fs::exists(fs::path(dir_path) / ".git")) {
			gint ec_dummy = 0;
			std::string salida_rama = GestorGit::ejecutarComandoGit("rev-parse --abbrev-ref HEAD", dir_path, "", &ec_dummy);
...
void Intefaz::run_git_sync(GtkWidget *boton, gpointer user_data) {
	...
	std::thread t([=]() mutable {
		try {
			...
			ResultadoOperacionGit pushResult = GestorGit::subirCambios(dir_path, ramaEfectiva, mensaje, token, url_repo);
			if (pushResult.exito) {
...
void Intefaz::run_git_compiler(GtkWidget *boton, gpointer user_data) {
	...
	if (g_spawn_command_line_sync(cmd_init.c_str(), &output_init, &stderr_init, &exit_status_init, &err_init)) {
	...
	if (output_verify) {
		g_free(output_verify);
	}
	if (stderr_verify) {
		g_free(stderr_verify);
	}
...
void Intefaz::build_interface(GtkApplication *app, gpointer user_data) {
	...
	gtk_widget_set_tooltip_text(combo_rama, "Selecciona una rama existente del desplegable\no escribe el nombre de una rama nueva para crearla");
	gtk_widget_set_hexpand(combo_rama, TRUE);

	btn_cargar_ramas = gtk_button_new_with_label("🔄");
	gtk_widget_set_tooltip_text(btn_cargar_ramas, "Cargar ramas remotas existentes desde GitHub\n(usa la URL y el Token actuales)");
	...
	gtk_css_provider_load_from_string(provider, ".log-mono {"
